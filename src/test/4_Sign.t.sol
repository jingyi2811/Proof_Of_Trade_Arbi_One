...
//    }
    ...
    function test4_3() public {
        address alice = vm.addr(1); // Set alice as signer
        Verifier verifier = new Verifier(alice);

        string memory timestamp = Strings.toString(block.timestamp);
        uint amount = 3;
        address msgSender = address(0x11);

        bytes32 msgHash = keccak256(abi.encode(timestamp, amount, msgSender)).toEthSignedMessageHash();
        ...
        verifier.verifyV1(timestamp, amount, msgSender, r, s, v);
        verifier.verifyV2(timestamp, amount, msgSender, signature);
    }
